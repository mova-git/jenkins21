   ...
   environment {
    ...
    stages {
        ...
        steps {
            ...
            sh '''
            ssh -o StrictHostKeyChecking=no ubuntu@13.201.89.218 '
              sudo apt update &&
            ...
            sh '''
            scp -o StrictHostKeyChecking=no target/app.jar ubuntu@13.201.89.218:/tmp/app.jar
            '''

            // Step 3: Move JAR to Apache path (usually not ideal, but for demo it’s fine)
            sh '''
            ssh -o StrictHostKeyChecking=no ubuntu@13.201.89.218 "sudo mv /tmp/app.jar /var/www/html/app.jar"
            '''
